...
Elevator.prototype.decide = function() {
    ...
    if(elevator.id == '1'){
        // console.log('elevator is ' + JSON.stringify(elevator) + ' in floor ' + elevator.at_floor() + ' to floor ' + elevator.get_destination_floor() + ' now in Position ' + elevator.get_position())
        console.log('at floor : ' + elevator.at_floor() + '; people inside ' + JSON.stringify(people) + ', total : ' + people.length)
    }
    ...
    if(person) {
        var dest_floor = people.map(function(o){return o.destination_floor;})
        var target_floor = arrayUnique(dest_floor.concat(requests));

        var closest_floor = target_floor[0];
        var diff = Math.abs (elevator.at_floor() - closest_floor);
        for (var val = 0; val < target_floor.length; val++) {
            var newdiff = Math.abs (elevator.at_floor() - target_floor[val]);
            if (newdiff < diff) {
                diff = newdiff;
                closest_floor = target_floor[val];
            }
        }

        if(elevator.id == '1') console.log('target floor ' + target_floor)
        return this.commit_decision(closest_floor);

        if(elevator.at_floor() > closest_floor){
